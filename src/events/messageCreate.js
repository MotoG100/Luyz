...
`)
 if(!cmd.isOn) return message.reply(`${m.feliz}⟩ me desculpe pelo aviso, mas o comando **${command}** está desligado, possivelmente deve ser uma manutenção, espere até que o problema se resolva.`)
send({
  t: 'custom',
  custom: {
...
if(!message.member.permissions.has(cmd.perms || [])) return message.reply(`${m.no}⟩ eu preciso da permissão \`${cmd.perms || []}\` para executar o comando **${command}**.`)

if(!message.guild.members.me.permissions.has(cmd.perms || [])) return message.reply(`${m.no}⟩ eu preciso da permissão \`${cmd.perms || []}\` para executar o comando **${command}**.`)
